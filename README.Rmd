...
This package is heavily inspired by and derived in part from the [fiftystater package](https://cran.r-project.org/package=fiftystater) by William Murphy. In contrast, `urbnmapr`:
  ...
  - Converts the shapefile data `sf` format
...
sf::st_geometry(get_urbn_labels(map = "states", sf = TRUE))

states_sf %>%
  ggplot() +
  geom_sf(aes(), 
          fill = "grey", color = "#ffffff", size = 0.25) +
  geom_sf(data = get_urbn_labels(map = "states", sf = TRUE), aes(), 
            size = 3)
```

If you want to label a different geography, use `geom_sf_label`.
